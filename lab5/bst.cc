...
bool delete_node(Node*& root, int key) {
  ...
  // case 3: target has two children
     ...
     */
      ...
      int key_p = predecessor->key;
      delete_node(root, key_p);
   ...
   */
   ...
   if (root == NULL) {
       return 0;
        ...
        i = 1 + numNodes(root->left) + numNodes(root->right);
   }
    return i;
   ...
   int i = 0;
    if (root == 0) {
        ...
        if ((root->left == NULL)&&(root->right == NULL)) {
            ...
            i = numLeaves(root->left) + numLeaves(root->right);
        }
    }
    return i;
...
void runTests(Node*& tree, std::vector<int> keys) {
  ...
  unit.assertEquals("Height aaaaaaa", 3, height(tree));
  ...
  unit.assertEquals("Number of nodes (Delete a leaf)", 8, numNodes(tree));
  unit.assertEquals("Number of leaves (Delete a leaf)", 3, numLeaves(tree));
  unit.assertEquals("Height (Delete a leaf)", 3, height(tree));
  ...
  unit.assertEquals("Number of nodes (Delete a node with one child)", 7, numNodes(tree));
  unit.assertEquals("Number of leaves (Delete a node with one child)", 3, numLeaves(tree));
  unit.assertEquals("Height (Delete a node with one child)", 3, height(tree));
  ...
  unit.assertEquals("Number of nodes (Delete a node with two children)", 6, numNodes(tree));
  unit.assertEquals("Number of leaves (Delete a node with two children)", 2, numLeaves(tree));
  unit.assertEquals("Height (Delete a node with two children)", 3, height(tree));

  //Delete root
  unit.assertNonNull("Finding 5 (the root)", find(tree, 5));
  delete_node(tree, 5);
  unit.assertNull("Deleting 5 (the root)", find(tree, 5));
  unit.assertEquals("Number of nodes (Delete root)", 5, numNodes(tree));
  unit.assertEquals("Number of leaves (Delete root)", 2, numLeaves(tree));
  unit.assertEquals("Height (Delete root)", 3, height(tree));
  ...
  delete_node(tree2, 7);
  unit.assertEquals("Number of nodes (delete root with 2 children) ", 3, numNodes(tree2));
  unit.assertEquals("Number of leaves (delete root with 2 children) ", 2, numLeaves(tree2));
  unit.assertEquals("Height (delete root with 2 children) ", 1, height(tree2));
	
  // delete root with 2 children, left child of root is not predecessor, predecessor has 1 child
  insert(tree2, 4);
  insert(tree2, 2);
  delete_node(tree2, 5);
  unit.assertEquals("Number of nodes (delete root with 2 children, np)", 4, numNodes(tree2));
  unit.assertEquals("Number of leaves (delete root with 2 children, np)", 2, numLeaves(tree2));
  unit.assertEquals("Height (delete root with 2 children, np)", 2, height(tree2));
