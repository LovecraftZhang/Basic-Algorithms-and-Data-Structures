...
int goOver(int *pInt, int size, int node, int* i);
...
void printHeap(int *heap, int size, int node = 0, int d = 0) {
    // Q1 way-1
//    if (size != 0 ) {
//        for (node; node <= size; node = goOver(heap, size, node, &d)) {
//            for (int i = 0; i < d; i++) {
//                cout << "*";
//            }
...
//        cout<< heap[node] << endl;
//        if (2 * node + 1 < size) {
//            printHeap(heap, size, 2*node + 1, d + 1); // d + 1 actually does not pass "d" to function
//        }
//
//        if (2* node + 2 < size) {
...
int goOver(int *pInt, int size, int node, int *i) {
    if ((2 * node + 1) <= size) {
        ++*i;
        return 2 * node + 1;
    } else if (2 * node + 2 <= size) {
        ++*i;
        return 2 * node + 2;
    } else {
        while ((node <= size) && (node > 0)) {
            if ((node - 1) / 2 < 0) {
                return size + 1;
            }

            if ((node + 1 < size) && (node % 2 != 0)) {
                return node + 1;
            }
            node = (node - 1) / 2;
            --*i;
