...
/*-- LQueue.h -------------------------------------------------------------
  ...
  Basic operations:
    ...
    move_to_front:    Move a element to front
    merge_two_queues: Merge two queues and keep it sorted
    move_after_front: Move the last element in the queue to the position just after front
...
class Queue
{
 public:
  ...
  /*-----------------------------------------------------------------------
  A function to search through a queue for a particular key value,
  and if found, move the node to the front of the same queue
  -----------------------------------------------------------------------*/

  void merge_two_queues(Queue & original);
  /*-----------------------------------------------------------------------
   Taking two queues that are each ordered according to a given field,
   and merges the two queues into one queue whose absolute order of the
   merged elements is kept.
  -----------------------------------------------------------------------*/

  void move_after_front ();
  /*-----------------------------------------------------------------------
  Move the node in the back to the position just after front.
  -----------------------------------------------------------------------*/
