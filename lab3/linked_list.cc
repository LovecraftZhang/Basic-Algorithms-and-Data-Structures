  ...
  // ******** WRITE YOUR CODE HERE ********
    if (head == NULL) {
        ...
        if (head->key == oldKey) {
            ...
            insert_after(temp, oldKey, newKey);
        }
    }
...
Node* interleave(Node* list1, Node* list2){
  ...
  if (list1 == NULL) {
      ...
      Node *list = new Node(), *l1 = list1,*l2 = list2, *temp = list;
      bool list1end = false, list2end = false;
      for (int i = size(list1), j = size(list2); i > 0, j > 0; i--, j--) {
          if ((l1 != NULL) && (l2 != NULL)) {
              temp->key = l1->key;
              l1 = l1->next;
              temp->next = new Node();
          ...
          if ((l1 == NULL) && (l2 == NULL)) {
              ...
              return list;
          } else if ((list1end) && (l2 != NULL)) {
              for (int a = size(l2); a > 0; a--) {
                  temp->key = l2->key;
                  l2 = l2->next;
                  temp->next = new Node();
                  temp = temp->next;
              ...
              delete_last_element(list);
              return list;
          } else if ((list2end) && (l1 != NULL)) {
              for (int b = size(l1); b > 0; b--) {
                  temp->key = l1->key;
                  l1 = l1->next;
                  temp->next = new Node();
                  temp = temp->next;
